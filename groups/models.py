...
"""
Fedha-Grow — data processing consent
====================================
Documented consent for collecting and processing a person's personal data —
the lawful basis under the Data Protection Act 2011, and especially important
for ASSISTED onboarding, where an agent captures someone else's data from a
physical form.

Design:
  * One consent record per person, tied to a versioned consent STATEMENT — so
    "what did they agree to, and when" is always answerable, even as the
    statement text evolves.
  * Two capture paths: a signed PHYSICAL form (agent uploads the scan) or a
    DIGITAL agreement (person ticks consent when activating their account).
  * Printable: a blank form to sign before capture, and a filled record as
    proof of a given person's consent.

IMPORTANT: the consent STATEMENT text (in consent_statements.py) is a drafted
starting point and MUST be reviewed by a data-protection lawyer before use.
The mechanism here is sound; the exact wording needs legal sign-off.
"""
class DataProcessingConsent(models.Model):
    METHOD_CHOICES = [
        ("physical_form", "Signed physical form (assisted capture)"),
        ("digital",       "Digital agreement (self-service)"),
    ...
    # which version of the statement they agreed to (text lives in
    # consent_statements.py, keyed by version) — so we always know exactly
    ...
    # assisted capture: the agent who captured this, and the signed scan.
    captured_by = models.ForeignKey(
        "borrowers.BorrowerProfile", on_delete=models.SET_NULL,
        null=True, blank=True, related_name="consents_captured",
        help_text="The agent/sub-admin who captured this consent, if assisted.")
    signed_form = models.FileField(
        upload_to="consent_forms/", null=True, blank=True,
        help_text="Scan/photo of the signed physical consent form.")
    witness_name = models.CharField(max_length=200, blank=True)

    # withdrawal (DPA right to withdraw consent)
    ...
    class Meta:
        ordering = ["-date"]
        indexes = [models.Index(fields=["borrower", "given", "withdrawn"])]

    def __str__(self):
        state = "withdrawn" if self.withdrawn else ("given" if self.given else "not given")
        return f"Consent ({state}) — {self.borrower.full_name} v{self.statement_version}"

    @property
    def is_active(self) -> bool:
        """Consent currently in force: given and not withdrawn."""
        return self.given and not self.withdrawn

    def withdraw(self):
        self.withdrawn = True
        self.withdrawn_date = timezone.now()
        self.save(update_fields=["withdrawn", "withdrawn_date"])

    @classmethod
    def active_for(cls, borrower):
        """The person's current active consent, if any."""
        return cls.objects.filter(borrower=borrower, given=True, withdrawn=False).first()
