...
class ScheduleIGenerator:
    ...
    def generate(self):
        template_path = 'compliance/templates/schedule_i_template.docx'
        
        # Use python-docx-template for Word doc generation
        doc = DocxTemplate(template_path)
        
        context = {
            # Section 1: Applicant Information
            'company_name': self.data['lender_profile']['company_name'],
            'registration_number': self.data['lender_profile']['registration_number'],
            'physical_address': self.data['lender_profile']['physical_address'],
            'establishment_date': self.data['lender_profile']['establishment_date'],
            
            # Section 2: Principal Activities (auto-generated based on tier)
            'principal_activities': self._generate_activities(),
            
            # Section 3: Capital Structure
            'authorized_capital': self.data['lender_profile']['stated_capital'],
            'issued_capital': self.data['financial_data']['issued_capital'],
            'shareholding_structure': self._generate_shareholding(),
            
            # Section 4: Financial Projections (AI-generated)
            'year_1_projections': self._generate_projections(1),
            'year_2_projections': self._generate_projections(2),
            'year_3_projections': self._generate_projections(3),
            
            # Section 5: Management Structure
            'board_members': self._format_board_members(),
            'key_personnel': self._format_key_personnel(),
        }
...
class BusinessPlanGenerator:
    def __init__(self, lender_data):
        self.data = lender_data
        
    def generate_executive_summary(self):
        prompt = f"""
        Generate a professional executive summary for a {self.data['cbl_tier']} MFI with:
        - Company: {self.data['company_name']}
        - Capital: M{self.data['stated_capital']:,}
        - Target: {self.data['target_market']}
        - Location: {self.data['city']}, Lesotho
        
        Include: mission, market opportunity, competitive advantage, financial highlights
        Length: 2-3 paragraphs, professional tone for CBL review
        """
    ...
    def generate_market_analysis(self):
        # Use Lesotho microfinance market data
        market_data = self._fetch_market_data()
        
        prompt = f"""
        Create a market analysis section for a Lesotho MFI business plan:
        - Market size: {market_data['market_size']}
        - Growth rate: {market_data['growth_rate']}
        - Key competitors: {market_data['competitors']}
        - Target segment: {self.data['target_market']}
        
        Include regulatory environment, market trends, customer needs
        """
        
        return self._call_ai_service(prompt)
    
    def generate_financial_projections(self):
        # Create realistic projections based on tier and capital
        model = FinancialProjectionModel(
            capital=self.data['stated_capital'],
            tier=self.data['cbl_tier'],
            market_data=self._fetch_market_data()
        )
        
        return {
            'revenue_projection': model.project_revenue(3),
            'expense_projection': model.project_expenses(3), 
            'cash_flow': model.project_cash_flow(3),
...
class FormPreFiller:
    ...
    def pre_fill_schedule_ii(self):
        """Auto-populate Schedule II Information Sheet"""
        return {
            # Company Information
            'company_details': self._extract_company_info(),
            
            # Ownership Profile (auto-calculated)
            'ownership_breakdown': self._calculate_ownership(),
            'beneficial_owners': self._identify_beneficial_owners(),
            
            # Board Composition (from personnel data)
            'board_composition': self._format_board_composition(),
            'independence_analysis': self._analyze_board_independence(),
            
            # Group Structure (if applicable)
            'group_entities': self._map_group_structure(),
            'related_parties': self._identify_related_parties(),
        }
    
    def pre_fill_fit_proper_forms(self):
        """Auto-populate fit & proper questionnaires for all personnel"""
        completed_forms = []
        
        for person in self.lender.personnel.all():
            form_data = {
                'personal_info': self._extract_personal_info(person),
                'employment_history': self._format_employment_history(person),
                'business_affiliations': self._extract_affiliations(person),
                'declarations': self._pre_fill_declarations(person),
            }
            completed_forms.append(form_data)
...
class PolicyGenerator:
    def __init__(self, lender):
        self.lender = lender
        self.tier = lender.cbl_tier
        
    def generate_aml_policy(self):
        """Generate tier-specific AML/CFT policy"""
        base_template = self._load_template('aml_cft_base.docx')
        
        tier_requirements = {
            'tier1': ['enhanced_due_diligence', 'suspicious_transaction_monitoring', 'regular_compliance_audits'],
            'tier2': ['standard_due_diligence', 'transaction_monitoring', 'annual_compliance_review'],
            'tier3': ['basic_due_diligence', 'manual_monitoring', 'self_assessment'],
        }
        
        customizations = {
            'institution_name': self.lender.company_name,
            'compliance_officer': self._get_compliance_officer(),
            'risk_appetite': self._determine_risk_appetite(),
            'specific_procedures': tier_requirements[self.tier],
            'reporting_requirements': self._get_reporting_requirements(),
        }
        
        return self._customize_template(base_template, customizations)
    
    def generate_credit_policy(self):
        """Generate lending policy based on business model"""
        return {
            'lending_criteria': self._generate_lending_criteria(),
            'risk_assessment': self._generate_risk_framework(),
            'approval_matrix': self._generate_approval_matrix(),
            'collection_procedures': self._generate_collection_policy(),
        }
    

class AutoDocumentWorkflow:
    def __init__(self, lender):
        self.lender = lender
        self.generator = ComplianceDocumentGenerator(lender)
        
    def generate_application_package(self):
        """One-click generation of complete CBL application"""
        try:
            # Collect all data
            data_sources = self.generator.collect_data_sources()
            
            # Generate required documents based on tier
            documents = {}
            
            if self.lender.cbl_tier in ['tier1', 'tier2', 'tier3']:
                documents['schedule_i'] = ScheduleIGenerator(data_sources).generate()
                documents['schedule_ii'] = ScheduleIIGenerator(data_sources).generate()
                documents['business_plan'] = BusinessPlanGenerator(data_sources).generate()
                documents['aml_policy'] = PolicyGenerator(self.lender).generate_aml_policy()
                
                if self.lender.cbl_tier in ['tier1', 'tier2']:
                    documents['risk_manual'] = PolicyGenerator(self.lender).generate_risk_manual()
                    documents['board_charter'] = PolicyGenerator(self.lender).generate_board_charter()
                
                if self.lender.cbl_tier == 'tier1':
                    documents['audit_charter'] = PolicyGenerator(self.lender).generate_audit_charter()
                    documents['credit_committee'] = PolicyGenerator(self.lender).generate_credit_charter()
            
            # Generate fit & proper forms for all personnel
            for person in self.lender.personnel.all():
                documents[f'fit_proper_{person.id}'] = FitProperGenerator(person).generate()
            ...
            return {
                ...
                'message': f'Generated {len(documents)} compliance documents successfully'
            }
...
class DocumentValidator:
    ...
    def validate_schedule_i(self, document_data):
        """AI-powered validation of Schedule I completeness"""
        errors = []
        warnings = []
        
        # Required field validation
        required_fields = self._get_required_fields('schedule_i')
        for field in required_fields:
            if not document_data.get(field):
                errors.append(f"Missing required field: {field}")
        
        # Business logic validation
        if document_data.get('issued_capital') > document_data.get('authorized_capital'):
            errors.append("Issued capital cannot exceed authorized capital")
            
        # Consistency checks across documents
        consistency_issues = self._check_cross_document_consistency(document_data)
        ...
        return {
            ...
            'errors': errors,
            'warnings': warnings,
            'completeness_score': self._calculate_completeness(document_data)
        }
...
class DocumentTemplateManager:
    def __init__(self):
        self.templates_path = 'compliance/templates/'
        
    def get_tier_specific_template(self, document_type, tier):
        """Load appropriate template based on tier"""
        template_name = f"{document_type}_{tier}.docx"
        return self._load_template(template_name)
    
    def update_template_from_cbl(self, document_type):
        """Update templates when CBL changes requirements"""
        # This would integrate with CBL's API or scraping system
        latest_version = self._fetch_latest_cbl_template(document_type)
        self._update_local_template(document_type, latest_version)
        
    def validate_template_compatibility(self, template, data_schema):
        """Ensure template can be populated with available data"""
        required_fields = self._extract_template_fields(template)
        available_fields = data_schema.keys()
